...
/* Descriptive frequencies with chi-square tests of association
   (from progetto.sas, "stat descrittive" section). */
proc freq data=pp;
  tables low race smoke ptl ht ui /chisq;
run;
proc freq data=pp;
  tables race*smoke race*ht race*ui race*ptl smoke*ptl smoke*ui
         ht*smoke ptl*ui ht*ptl ht*ui
         low*ptl low*ht low*ui low*race low*smoke /chisq;
run;
